...
latestObservation conn cid ref = do
    rows <-
        query
            conn
            "SELECT country, source, indicator, year, value \
            \FROM observation \
            \WHERE country = ? AND source = ? AND indicator = ? \
            \ORDER BY year DESC \
            \LIMIT 1"
            (cid, refSource ref, refIndicator ref)
...
latestCountryValue conn cid iid =
    fmap toYearValue <$> latestObservation conn cid (indicatorRef iid)
  where
    toYearValue obs =
        YearValue
            (obsYear obs)
