...
exp7a ::
    [CountryId] -> [(RegionId, [CountryId])] -> IO (String, String)
...
--         gt4 = countryTables ++ [gnct, difft] :: [CountryPak3]
...
exp1a_ countries = do
    ...
    countryTables :: [TerryTable CountryId (Double)] <-
        mapM (\(d, y) -> lookupTable conn (d) y) reqYears
    close conn

    let aggs =
            map
                (\a -> map (aggregateTery a countries) countryTables)
                [sum1, min1, median1, max1, mean1, stdDev1]
